...
function compileFromURL(path: string) {
  ...
  const regexString = paths.reduce((prev, curr) => {
    if (!curr.startsWith(":")) {
      return prev + `\\/??${curr}`;
    }
...
function build() {
  ...
  if (fs.existsSync("index.html")) {
    ...
    IndexPage = fs.readFileSync(join(__dirname, "../assets", "index.html"), "utf8");
  }
...
function init() {
  if (!fs.existsSync("vercel.json")) {
    ...
    fs.copyFileSync(join(__dirname, "../assets", "vercel.example.json"), "vercel.json");
  }
  if (!fs.existsSync("redirects.yml")) {
    console.log("Generating example redirects.yml");

    fs.copyFileSync(join(__dirname, "../assets", "redirects.example.yml"), "redirects.yml");
  }
}
